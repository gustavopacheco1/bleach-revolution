...
Weapon::Weapon(LuaInterface *_interface) : Event(_interface)
{
	id = 0;
	level = 0;
	magLevel = 0;
...
WeaponMelee::WeaponMelee(LuaInterface *_interface) : Weapon(_interface) {}
...
WeaponDistance::WeaponDistance(LuaInterface *_interface) : Weapon(_interface)
{
	hitChance = -1;
...
bool WeaponDistance::useWeapon(Player *player, Item *item, Creature *target) const
{
	...
	if (chance == -1)
	{
		...
		uint32_t distance = std::max(std::abs(playerPos.x - targetPos.x), std::abs(
				 ...
				 skill = player->getSkill(SKILL_DIST, SKILL_LEVEL);
		if (maxHitChance == 75)
		{
...
WeaponWand::WeaponWand(LuaInterface *_interface) : Weapon(_interface)
{
	minChange = 0;
	maxChange = 0;
