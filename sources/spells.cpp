...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	{
		std::string tmpStrValue = asLowerCaseString(strValue);
		if (tmpStrValue == "attack" || tmpStrValue == "attacking" || tmpStrValue == "heal" || tmpStrValue == "healing" || tmpStrValue == "support" || tmpStrValue == "supporting" || tmpStrValue == "special" || tmpStrValue == "ultimate")
		...
		if (position.z != floor)
		{
			...
			if (!tmpTile || (!tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID)))
			{
				...
				if (tile && tmpTile && tmpTile->ground && !tmpTile->hasProperty(IMMOVABLEBLOCKSOLID) &&
					!tmpTile->floorChange() && tile->hasFlag(TILESTATE_HOUSE) == tmpTile->hasFlag(TILESTATE_HOUSE) && tile->hasFlag(TILESTATE_PROTECTIONZONE) == tmpTile->hasFlag(TILESTATE_PROTECTIONZONE))
...
ConjureSpell::ConjureSpell(LuaInterface *_interface) : InstantSpell(_interface)
{
	isAggressive = false;
	conjureId = 0;
	conjureCount = 1;
...
RuneSpell::RuneSpell(LuaInterface *_interface) : Action(_interface)
{
	runeId = 0;
	function = NULL;
