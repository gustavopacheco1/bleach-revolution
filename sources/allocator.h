...
#ifdef _MSC_VER
void *operator new[](size_t bytes, int32_t dummy) void operator delete(void *p, int32_t dummy);
...
private:
	...
	typedef std::map<size_t, boost::pool<boost::default_user_allocator_malloc_free> *, std::less<size_t>,
					 dummyallocator<std::pair<const size_t, boost::pool<boost::default_user_allocator_malloc_free> *>>>
		Pools;
	Pools pools;
