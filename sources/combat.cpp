...
ReturnValue Combat::canDoCombat(const Creature *attacker, const Creature *target, bool isAggressive)
{
	...
	return checkZones && (target->getTile()->hasFlag(TILESTATE_OPTIONALZONE) ||
						  (attacker->getTile()->hasFlag(TILESTATE_OPTIONALZONE) && !target->getTile()->hasFlag(TILESTATE_OPTIONALZONE) &&
...
bool Combat::CombatDispelFunc(Creature *caster, Creature *target, const CombatParams &params, void *)
{
	...
	if (params.dispelType == CONDITION_INVISIBLE)
	{
		if (Player *player = target->getPlayer())
		{
			...
			if (item && item->getID() == ITEM_STEALTH_RING && (g_game.getWorldType(casterPlayer, player) == WORLDTYPE_HARDCORE || player->getTile()->hasFlag(TILESTATE_HARDCOREZONE)) && random_range(1, 100) <= 10)
...
void Combat::combatTileEffects(const SpectatorVec &list, Creature *caster, Tile *tile, const CombatParams &params)
{
	...
	if (params.itemId)
	{
		...
		{
			bool pzLock = false;
			if (((g_game.getWorldType(player) == WORLDTYPE_OPTIONAL || g_config.getBool(ConfigManager::OPTIONAL_PROTECTION)) && !tile->hasFlag(TILESTATE_HARDCOREZONE)) || tile->hasFlag(TILESTATE_OPTIONALZONE))
			{
				switch (itemId)
				{
				case ITEM_FIREFIELD:
