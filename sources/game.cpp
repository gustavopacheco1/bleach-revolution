...
Creature *Game::getCreatureByID(const uint32_t &id)
{
	...
	{ // id belongs to a player
		return getPlayerByID(id);
	}
	else if (id <= Monster::monsterAutoID)
	{ // id belongs to a monster
...
bool Game::removeMoney(Cylinder *cylinder, int64_t money, uint32_t flags /*= 0*/, bool canDrop /*= true*/, bool ignoreBank /*= false*/)
{
	...
	if (!ignoreBank && money > 0 && player && (int32_t)player->balance >= money)
	{
		...
		{
			msg << "Foi pago " << money << " ryo da conta do banco. Agora o seu saldo banc�rio � " << player->balance << " ryo.";
		}
...
bool Game::playerRotateItem(const uint32_t &playerId, const Position &pos, int16_t stackpos, const uint16_t &spriteId)
{
	...
	Item *item = thing->getItem();
	if (!item || item->getClientID() != spriteId || !item->isRoteable() || (item->isLoadedFromMap() && (item->getUniqueId() || (item->getActionId() && item->getContainer()))))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
							  ...
							  const uint32_t &tradePlayerId, const uint16_t &spriteId)
{
	...
	if (!tradeItem || tradeItem->getClientID() != spriteId || !tradeItem->isPickupable() || (tradeItem->isLoadedFromMap() && (tradeItem->getUniqueId() || (tradeItem->getActionId() && tradeItem->getContainer()))))
	{
		player->sendCancelMessage(RET_NOTPOSSIBLE);
		return false;
	}
			...
			if (damage > 0)
			{
				...
				if (textColor < COLOR_NONE)
				{
					...
					if (elementDamage)
					{
						...
						if (target->getPosition() != creaturePos)
						{ // The target was teleported/moved on statschange, a new spectator list must be created
							SpectatorVec newList;
							getSpectators(newList, targetPos, true, true);
							addMagicEffect(newList, targetPos, magicEffect);
						}
