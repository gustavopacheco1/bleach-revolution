...
TalkActions::TalkActions() : m_interface("TalkAction Interface")
{
...
bool TalkActions::onPlayerSay(Creature *creature, uint16_t channelId, const std::string &words, bool ignoreAccess)
{
	...
	if (player)
	{
		...
		if ((!ignoreAccess && std::find(exceptions.begin(), exceptions.end(), asLowerCaseString(player->getName())) == exceptions.end() && (talkAction->getAccess() > player->getAccess() || (talkAction->hasGroups() && !talkAction->hasGroup(player->getGroupId())))) || player->isAccountManager())
		{
...
TalkAction::TalkAction(LuaInterface *_interface) : Event(_interface)
{
	m_function = NULL;
...
TalkAction::TalkAction(const TalkAction *copy) : Event(copy)
{
...
bool TalkAction::thingProporties(Creature *creature, const std::string &, const std::string &param)
{
	...
	for (tokenizer::iterator it = tokens.begin(); it != tokens.end();)
	{
		...
		if (Item *item = thing->getItem())
		{
			...
			else if (action == "destination" || action == "position" || action == "pos" || action == "dest" || action == "location" || action == "loc") // TODO: doesn't work
			{
...
bool TalkAction::banishmentInfo(Creature *creature, const std::string &, const std::string &param)
{
	...
	ss << what << " has been " << (deletion ? "deleted" : "banished") << " at:\n"
	   << formatDateEx(ban.added, "%d %b %Y") << " by: " << admin << ".\nThe comment given was:\n"
	   << ban.comment << ".\n"
