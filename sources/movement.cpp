...
MoveEvents::MoveEvents() : m_lastCacheTile(NULL)
{
...
bool MoveEvents::hasEquipEvent(Item *item)
{
	...
	return (event = getEvent(item, uniqueId, actionId, MOVE_EVENT_EQUIP)) && !event->isScripted() && (event = getEvent(item, uniqueId, actionId, MOVE_EVENT_DE_EQUIP)) && !event->isScripted();
}
...
bool MoveEvents::hasTileEvent(Item *item)
{
	...
	return getEvent(item, uniqueId, actionId, MOVE_EVENT_STEP_IN) || getEvent(item, uniqueId, actionId, MOVE_EVENT_STEP_OUT) || getEvent(item, uniqueId, actionId, MOVE_EVENT_ADD_TILEITEM) || getEvent(item, uniqueId, actionId, MOVE_EVENT_REMOVE_TILEITEM);
}
...
uint32_t MoveEvents::onItemMove(Creature *actor, Item *item, Tile *tile, bool isAdd)
{
	...
	if (m_lastCacheTile == tile)
	{
		...
		for (uint32_t i = 0; i < m_lastCacheItemVector.size(); ++i)
		{
			if ((tileItem = m_lastCacheItemVector[i]) && tileItem != item && (moveEvent = getEvent(tileItem, tileItem->getUniqueId(), tileItem->getActionId(), tileEventType)))
...
MoveEvent::MoveEvent(LuaInterface *_interface) : Event(_interface)
{
	stepFunction = NULL;
	moveFunction = NULL;
	equipFunction = NULL;
...
MoveEvent::MoveEvent(const MoveEvent *copy) : Event(copy)
{
