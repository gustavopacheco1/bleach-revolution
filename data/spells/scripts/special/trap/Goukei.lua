...
function onCastSpell(cid, var)
    ...
    if exhaustion.check(cid, 13301) == TRUE then
        if exhaustion.get(cid, 13301) >= 120 then
            doPlayerSendCancel(cid, "Doton Doryu Heki cooldown " .. math.floor(exhaustion.get(cid, 13301) / 60) ..
                " minutos.")
            return false
        end
        if exhaustion.get(cid, 13301) > 60 then
            doPlayerSendCancel(cid, "Doton Doryu Heki cooldown 1 minuto and " .. (exhaustion.get(cid, 13301) - 60) ..
                " segundos.")
            return false
        end
