...
local function getClosestTarget(cid, current_target, previous_target)
    ...
    if getDistanceBetween(cid_position, getCreaturePosition(previous_target)) < getDistanceBetween(cid_position, getCreaturePosition(current_target)) then
...
function onCastSpell(cid, var)
    ...
    for _, spectator in ipairs(getSpectators(player_position, 7, 5)) do
        if spectator ~= cid and getCreatureStorage(spectator, "dead_player_id") ~= -1 and isSightClear(player_position, getCreaturePosition(spectator), false) then
            if param == "all" then
