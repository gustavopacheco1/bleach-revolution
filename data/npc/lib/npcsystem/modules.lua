...
if (Modules == nil) then
	...
	function StdModule.learnSpell(cid, message, keywords, parameters, node)
		...
		if (isPremium(cid) or not (parameters.premium)) then
			...
			elseif (getPlayerLevel(cid) < parameters.level) then
				npcHandler:say('You need to obtain a level of ' ..
			...
			elseif (not doPlayerRemoveMoney(cid, parameters.price)) then
				npcHandler:say('You do not have enough money, this spell costs ' .. parameters.price .. ' ryo.', cid)
			else
	...
	function StdModule.bless(cid, message, keywords, parameters, node)
		...
		if (isPremium(cid) or not getBooleanFromString(getConfigValue('blessingsOnlyPremium')) or not parameters.premium) then
			...
			if (getPlayerLevel(cid) > parameters.startLevel) then
				price = (
					price + ((math.min(parameters.endLevel, getPlayerLevel(cid)) - parameters.startLevel) * parameters.levelCost))
			end
	...
	function TravelModule:parseDestinations(data)
		for destination in string.gmatch(data, '[^;]+') do
			...
			if (name ~= nil and pos.x ~= nil and pos.y ~= nil and pos.z ~= nil and cost ~= nil) then
				...
				print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'Parameter(s) missing for travel destination:'
					, name, pos, cost, premium)
			end
		end
	end
	...
	function TravelModule.travel(cid, message, keywords, parameters, node)
		...
		if getPlayerStorageValue(cid, "language") == "en" then
			module.npcHandler:say('Do you want to travel to ' .. keywords[1] .. ' for ' .. parameters.cost .. ' ryo?', cid)
		else
			module.npcHandler:say('Você deseja viajar até ' .. keywords[1] .. ' por ' .. parameters.cost .. ' ryo?', cid)
		end
		return true
	end
	...
	function TravelModule.onDecline(cid, message, keywords, parameters, node)
		...
		module.npcHandler:say(module.npcHandler:parseMessage(module.npcHandler:getMessage(cid, MESSAGE_DECLINE),
	...
	function TravelModule.bring(cid, message, keywords, parameters, node)
		...
		if (
			(isPremium(cid) or not parameters.premium) and not isPlayerPzLocked(cid) and doPlayerRemoveMoney(cid, parameters.cost)
			) then
			module.npcHandler:say('Set the sails!', cid)
			module.npcHandler:releaseFocus(cid)
	...
	function OutfitModule:parseList(keywords, data)
		...
		for list in string.gmatch(data, '[^;]+') do
			...
			for tmp in string.gmatch(list, '[^,]+') do
				...
				elseif (e == 4) then
					...
					print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:',
						'Unknown parameter found in outfit list while parsing ' .. (outfit == nil and 'outfit' or 'item') .. '.', tmp, list)
				end
			...
			elseif (a ~= nil) then
				...
				if ((tmp ~= nil or tostring(a) == "money") and b ~= nil and c ~= nil) then
					...
					print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'Missing parameter(s) for outfit items.', b, c
						, d)
				end
			else
		...
		if (type(outfit) == 'table') then
			...
			if (tmp and table.maxn(items) > 0) then
				...
				print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'Invalid outfit, addon or empty items pool.',
					data)
			end
		end
	end
	...
	function OutfitModule.obtain(cid, message, keywords, parameters, node)
		...
		module.npcHandler:say('Do you want ' ..
			keywords[1] .. ' ' .. (addon == 0 and "outfit" or "addon") .. ' for ' .. items .. '?', cid)
		return true
	...
	function ShopModule:parseBuyable(data)
		for item in string.gmatch(data, '[^;]+') do
			...
			elseif (name ~= nil and itemid ~= nil and cost ~= nil) then
				...
				print('[Warning - ' .. getCreatureName(getNpcId()) .. '] NpcSystem:', 'Parameter(s) missing for item:', name, itemid
					, cost)
			end
		end
	end
	...
	function ShopModule.messageMatcher(keywords, message)
		for i, word in pairs(keywords) do
			if (
				type(word) == 'string' and string.find(message, word) and not string.find(message, '[%w+]' .. word) and
					not string.find(message, word .. '[%w+]')) then
				return true
			end
		end
