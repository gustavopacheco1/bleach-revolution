...
function onCreatureSay(cid, type, msg)
	...
	if talkState[talkUser] == 1 and tonumber(msg) then
		...
		if exhaustion.check(cid, "trainer_shielding") then
			...
			if msg < (trainer_time / 3600) then
				selfSayMultiLanguage(
					"You can't buy a training time lower than the one that you already have. You still have " ..
					format_trainer_time .. " for training your dodge.",
					"Você não pode comprar um tempo de treino menor do que o que você já tem. Você ainda tem " ..
					format_trainer_time .. " para trainer sua esquiva.",
					cid
				)
				return false
			end
	...
	if isInArray({ "yes", "sim" }, msg) and talkState[talkUser] == 2 then
		...
		selfSayMultiLanguage(
			"Thank you very much for collaborating with the Karakura Academy. Now you can train your dodge for " ..
			playerTrainerTime[cid] .. " hours.",
			"Muito obrigado por colaborar com a Academia de Karakura. Agora você pode treinar sua esquiva por " ..
			playerTrainerTime[cid] .. " horas.",
			cid
		)
