	...
	if not npcHandler:isFocused(cid) and getDistanceBetween(getThingPos(cid), getNpcPos()) < 5 then
		if isInArray({ "hi", "hello", "oi", "olá" }, msg) then
			selfSayMultiLanguage(
				...
				"Olá, posso dar-lhe acesso para entrar nessa porta, por apenas 2 {almas}.",
				cid
			)
	...
	if isInArray({ "yes", "sim" }, msg) then
		if getPlayerLevel(cid) < 100 then
			selfSayMultiLanguage(
				...
				"Você é muito fraco comparado com o lugar que eu te levaria. Quando você alcançar, pelo menos, o nível 100 venha até aqui novamente."
				,
				cid
			)
			npcHandler:releaseFocus(cid)
