...
function onStartup()
	...
	if (result:getID() ~= -1) then
		repeat
			data[result:getDataInt("id")] = { result:getDataInt("ownerid"), result:getDataInt("creationdata"),
				result:getDataInt("checkdata") }
		...
		if (created < (time - config.creationTime)) then
			...
			if (result:getID() ~= -1) then
				...
				for k, v in ipairs(ranks) do
					...
					if (v == 2) then
						result = db.getResult("SELECT `p`.`id` FROM `players` p LEFT JOIN `accounts` a ON `p`.`account_id` = `a`.`id` WHERE `p`.`rank_id` = "
				...
				if (members[2] < config.viceCount or members[4] < config.memberCount) then
					...
					elseif (check < time) then
						...
						db.executeQuery("UPDATE `players` SET `rank_id` = 0, `guildnick` = '', `guildjoin` = 0 WHERE `rank_id` IN (" ..
							tmp:sub(1, -2) .. ");")
