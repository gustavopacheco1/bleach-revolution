...
function onSay(cid, words, param, channel)
	...
	if(getPlayerGuildLevel(cid) == GUILDLEVEL_LEADER and isInArray({ 'withdraw' }, t[1])) then
		if(t[1] == 'pick') then
	...
	elseif(t[1] == 'deposit') then
		...
		doPlayerSendChannelMessage(cid, '', 'Current balance of guild ' .. result:getDataString('name') .. ' is: ' .. result:getDataLong('balance') .. ' bronze berries.', TALKTYPE_CHANNEL_W, CHANNEL_GUILD)
		result:free()
	end
