...
if (tonumber(check:getDataString("group_id")) >= getPlayerAccess(cid)) then
doPlayerSendTextMessage(cid, MESSAGE_STATUS_CONSOLE_BLUE, "Voce nao pode ver a conta de alguem igual ou superior a voce!")
return TRUE
end

local pass = db.getResult("SELECT name, password, key FROM accounts WHERE id = "..check:getDataString("account_id").."")
